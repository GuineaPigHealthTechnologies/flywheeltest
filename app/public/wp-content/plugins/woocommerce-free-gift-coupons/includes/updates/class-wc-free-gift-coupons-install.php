...
class WC_Free_Gift_Coupons_Install {
	...
	public static function maybe_update() {
		if ( ! empty( $_GET[ 'force_wc_fgc_db_update' ] ) && wp_verify_nonce( $_GET[ '_wc_fgc_admin_nonce' ], 'wc_fgc_force_db_update_nonce' ) ) {
			if ( self::can_update() && self::must_update() ) {
				self::force_update();
			}
		} elseif ( ! empty( $_GET[ 'trigger_wc_fgc_db_update' ] ) && wp_verify_nonce( $_GET[ '_wc_fgc_admin_nonce' ], 'wc_fgc_trigger_db_update_nonce' ) ) {
			if ( self::can_update() && self::must_update() ) {
				self::trigger_update();
			}
		}

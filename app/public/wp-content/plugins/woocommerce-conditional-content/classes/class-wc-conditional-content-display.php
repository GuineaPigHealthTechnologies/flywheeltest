...
class WC_Conditional_Content_Display {
		...
		if ( $contents && count( $contents ) ) {
			...
			foreach ( $contents as $content ) {
				...
				if ( $display ) {
					...
					if ( $custom_hook ) {
						$hook = apply_filters( 'woocommerce_conditional_content_get_output_hook', array( 'action'   => $custom_hook,
						...
						$hook         = apply_filters( 'woocommerce_conditional_content_get_output_hook', array( 'action'   => $hook_setting['action'],
	...
	public function template_display( $content_id = 0, $echo = true ) {
		if ( $content_id ) {
			$contents = get_posts( array( 'p'           => $content_id,
			                              'post_type'   => 'wccc',
			                              'post_status' => 'publish',
			                              'nopaging'    => true
			) );

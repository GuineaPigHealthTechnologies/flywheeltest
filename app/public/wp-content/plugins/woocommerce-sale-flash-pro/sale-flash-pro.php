 ...
 * Plugin updates
 */
woothemes_queue_update( plugin_basename( __FILE__ ), '7761d62beb597bea4f6fab56fef6739c', '18591' );

/**
 * Check if WooCommerce is active
 **/
add_action( 'plugins_loaded', 'woocommerce_sales_flash_init', 0 );

function woocommerce_sales_flash_init() {
	...
	class WC_Sale_Flash_Pro {
		...
		public function load_plugin_textdomain() {
			$locale = apply_filters( 'plugin_locale', get_locale(), 'wc_sale_flash' );
			$dir    = trailingslashit( WP_LANG_DIR );

			load_textdomain( 'wc_sale_flash', $dir . 'woocommerce-sale-flash-pro/wc_sale_flash-' . $locale . '.mo' );
			load_plugin_textdomain( 'wc_sale_flash', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
		}
		...
		function sale_flash( $html, $post, $product ) {
		...
		function sale_price_html( $html, $product ) {

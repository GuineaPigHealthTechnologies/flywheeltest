...
if ( is_woocommerce_active() && ! class_exists( 'WC_Store_Credit_Plus' ) ) {
	...
	define( 'WC_STORE_CREDIT_PLUS_VERSION', '2.1.16' );
	...
	class WC_Store_Credit_Plus {
		...
		public function __construct() {
			...
			add_filter( 'woocommerce_coupon_custom_discounts_array', array( $this, 'calculate_discrepancy' ), 10, 2 );
				...
				foreach ( $coupons as $code => $coupon ) {
					if ( 'store_credit' === self::get_coupon_prop( $coupon, 'type' ) ) {
						...
						if ( $credit_remaining <= 0 && 'yes' === get_option( 'woocommerce_delete_store_credit_after_usage', 'yes' ) ) {
							wp_trash_post( self::get_coupon_prop( $coupon, 'id' ) );
						} else {
		...
		public function coupon_get_discount_amount( $discount, $discounting_amount, $cart_item, $single, $coupon ) {
			...
			$applied_coupons = array_filter( WC()->cart->coupon_discount_totals, function ( $coupon_code ) use ( $coupon ) {
				return $coupon_code != $coupon->get_code();
			}, ARRAY_FILTER_USE_KEY );

			$already_applied_coupons = array_sum( $applied_coupons );
			...
			if ( ! is_null( $cart_item ) ) {
				...
				if ( $subtotal ) {
					...
					// For manual orders this can be null, so make sure we get correct product
					if ( is_null( $product ) ) {
						$product = $cart_item->get_product();
					}

					$price = version_compare( WC_VERSION, '3.0', '<' ) ? $product->get_price_excluding_tax() : wc_get_price_excluding_tax( $product, array( 'qty' => 1, 'price' => '' ) );

					$discount_percent = ( $price * $cart_item['quantity'] ) / $subtotal;
				}

				$discount = min( ( $coupon_amount * $discount_percent ) / $cart_item['quantity'], $discounting_amount );
			}
			return $discount;
		}
		...
		public function calculate_discrepancy( $discounts, $coupon ) {
			...
			$discrepancy = wc_add_number_precision( $coupon->get_amount() ) - array_sum( $discounts );

			if ( 0 == $discrepancy ) {
				return $discounts;
			}

			// There is a discrepancy, so now we need to smooth the values
			$discounts = array_map( function ( $value ) {
				return absint( $value );
			}, $discounts );

			// Calculate the new discrepancy after smoothing the values and add it to the first key
			$discrepancy              = wc_add_number_precision( $coupon->get_amount() ) - array_sum( $discounts );
			$first_key                = array_keys( $discounts );
			$first_key                = current( $first_key );
			$discounts[ $first_key ] += $discrepancy;

...
class WC_Social_Login extends SV_WC_Plugin {
	...
	public function add_admin_notices() {
		...
		if ( 1 === (int) get_transient( '_wc_social_login_hybridauth_caching_issue' ) ) {
			...
			$this->get_admin_notice_handler()->add_admin_notice(
				/* translators: Placeholders: %1$s - opening <a> HTML link tag, %2$s - closing </a> HTML link tag, %3$s cookie name wrapped in <code> HTML tags */
				sprintf( __( 'WooCommerce Social Login has detected a problem with your hosting configuration that prevents some customers from linking a social account. For Social Login to function as expected, please %1$sfollow the guidelines here%2$s, as your host must fix your site\'s caching rules to exclude %3$s cookies from caching.', 'woocommerce-social-login' ), '<a href="' . esc_url( wc_social_login()->get_documentation_url() . '#caching-issue' ) . '">', '</a>', '<code>wp_woocommerce_session*</code>' ),
		...
		if ( $customer instanceof WC_Customer ) {
			...
			if ( ! empty( $meta_data ) ) {
				...
				foreach ( array_keys( $meta_data ) as $meta_key ) {
					...
					if ( SV_WC_Helper::str_starts_with( $meta_key, '_wc_social_login' ) ) {

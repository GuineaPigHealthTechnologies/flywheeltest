...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.9.0', __( 'WooCommerce Nested Category Layout', 'woocommerce-nested-category-layout' ), __FILE__, 'init_woocommerce_nested_category_layout', array(
...
class WC_Nested_Category_Layout extends SV_WC_Plugin {
	...
	public function pre_get_posts( $q ) {
		...
		if (     ( ! $q->is_main_query() )
		      || ( ! $q->is_post_type_archive( 'product' ) && ! $q->is_tax( array_merge( array( 'product_cat', 'product_tag' ), wc_get_attribute_taxonomy_names() ) ) ) ) {
			return;
		}
		...
		if ( 'no' === get_option( 'woocommerce_nested_subcat_shop', 'no' ) ) {
			if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) {
				if ( $q->is_post_type_archive( 'product' ) || $q->is_page( wc_get_page_id( 'shop' ) ) ) {
					return;
				}
			} else {
				if ( $q->is_post_type_archive( 'product' ) || $q->is_page( woocommerce_get_page_id( 'shop' ) ) ) {
					return;
				}
			}
		}
	...
	public function list_categories_and_products() {
		...
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_3_0() ) {
			...
			if (  $wp_the_query->is_home() || ( $wp_the_query->is_front_page() && woocommerce_get_page_id( 'shop' ) !== (int) get_option( 'page_on_front' ) ) ) {
				return;
			}
		}
		...
		if (    ( isset( $woocommerce_loop['has_products'] ) && $woocommerce_loop['has_products'] )
			...
			if ( SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.3' ) && ( $wp_query->is_tax( 'product_cat' ) || is_shop() ) ) {
	...
	/**
	 * Fix the global query object as if it was altered from the archive-product.php template.
	 *
	 * @since 1.0
	 */

	...
	private function set_product_category_ids( $args ) {
		...
		if ( have_posts() ) {
			while ( have_posts() ) {
				...
				$this->product_category_ids[ SV_WC_Product_Compatibility::get_prop( $product, 'id' ) ] = array();
				...
				$product_category_ids = wp_get_post_terms( SV_WC_Product_Compatibility::get_prop( $product, 'id' ), 'product_cat', array( 'fields' => 'ids' ) );
				...
				foreach ( $product_category_ids as $category_id ) {
					if ( isset( $this->category_depths[ $category_id ] ) && $this->category_depths[ $category_id ] == $deepest[ $category_to_root[ $category_id ] ] ) {
						$this->product_category_ids[ SV_WC_Product_Compatibility::get_prop( $product, 'id' ) ][] = $category_id;
					}
				}

				// if this is empty it means the product is only in the current category, so add it
				if ( empty( $this->product_category_ids[ SV_WC_Product_Compatibility::get_prop( $product, 'id' ) ] ) && isset( $args['current_category'] ) ) {
					$this->product_category_ids[ SV_WC_Product_Compatibility::get_prop( $product, 'id' ) ][] = $args['current_category'];
				}

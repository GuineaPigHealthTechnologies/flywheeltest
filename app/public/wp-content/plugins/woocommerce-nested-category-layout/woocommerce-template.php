...
if ( ! function_exists( 'woocommerce_nested_category_products_content_section' ) ) {
	...
	function woocommerce_nested_category_products_content_section( $categories, $product_category_ids ) {
		...
		if ( $thumbnail_id ) {
			...
			$image = $image[0];
			echo apply_filters( 'wc_nested_category_layout_category_image', '<img src="' . $image . '" alt="' . $category->name . '" class="wc-nested-category-layout-category-image" style="padding-bottom: 10px;" />', $category );
		}

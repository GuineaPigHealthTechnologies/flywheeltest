...
if ( ! class_exists( 'Pie_WCWL_Custom_Tab' ) ) {
	...
	class Pie_WCWL_Custom_Tab {
		...
		public function add_waitlist_tab_to_product_options_panel() {
			include 'components/panel-side-tab.php';
		}
		...
		public function output_content_for_waitlist_panel() {
			if ( WooCommerce_Waitlist_Plugin::is_variable( $this->product ) ) {
				include 'components/panel-variable.php';
			} else {
				include 'components/panel-simple.php';
			}
		}
		...
		public function enqueue_scripts() {
			wp_enqueue_style( 'wcwl_admin_custom_tab_css', WCWL_ENQUEUE_PATH . '/includes/css/wcwl_admin_custom_tab.css', array(), WCWL_VERSION );
			wp_enqueue_script( 'wcwl_admin_custom_tab_js', WCWL_ENQUEUE_PATH . '/includes/js/wcwl_admin_custom_tab.js', array(), WCWL_VERSION, true );
			$data = $this->get_data_required_for_js();
			wp_localize_script( 'wcwl_admin_custom_tab_js', 'wcwl_tab', $data );
		}
		 ...
		 * @return array
		 */
		private function get_data_required_for_js() {
			return array(

...
if ( ! class_exists( 'Pie_WCWL_Frontend_Product' ) ) {
	...
	abstract class Pie_WCWL_Frontend_Product {
		...
		public function frontend_enqueue_scripts() {
			...
			wp_enqueue_style( 'wcwl_frontend', WCWL_ENQUEUE_PATH . '/includes/css/wcwl_frontend.css', array(), WCWL_VERSION );
		}
		 ...
		 * @since  1.8.0
		 */
		protected function setup_waitlist( $product_id = 0 ) {
			if ( ! $product_id ) {
				$product_id = $this->get_product_id();
			}
			if ( $this->has_wpml ) {
				$product_id = $this->get_main_product_id( $product_id );
			}
		...
		protected function get_product_id() {
			...
			if ( Pie_WCWL_Frontend_Init::is_ajax_variation_request() ) {
				if ( $this->has_wpml ) {
					$product_id = $this->get_main_product_id( absint( $_REQUEST['product_id'] ) );
				} else {
					$product_id = absint( $_REQUEST['product_id'] );
				}
			} elseif ( ! is_product() && ( ! empty( $post->post_content ) && strstr( $post->post_content, '[product_page' ) ) ) {
				$product_id = Pie_WCWL_Frontend_Init::get_product_id_from_post_content( $post->post_content );
			} else {
				$product_id = $post->ID;
			}

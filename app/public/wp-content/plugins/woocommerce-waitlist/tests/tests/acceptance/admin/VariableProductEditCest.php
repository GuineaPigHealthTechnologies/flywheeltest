...
class Admin_Variable_Product_Screen_Cest {

	/* WAITLIST TAB */
	public function WaitlistTabAppearsOnPage( AcceptanceTester $I ) {
		TestHelperFunctions::VisitWaitlistAdminTab( $I, 'variable', '' );
		$I->seeElement( '.wcwl_waitlist_menu_tab' );
	}

	public function ClickingWaitlistTabShowsWaitlistElements( AcceptanceTester $I ) {
		TestHelperFunctions::VisitWaitlistAdminTab( $I, 'variable' );
		TestHelperFunctions::SeeExpectedOutput( $I, array( 'Add new user', 'There are no users on the waiting list for this product.' ) );
	}
	...
	public function AddMultipleUsersToWaitlist( AcceptanceTester $I ) {
		...
		// Check for cross contamination between variations
		$I->click( '.wcwl_header_wrap:first-child' );
		$I->wait( 1 );
		$I->click( '.wcwl_variation_tab:nth-child(2) .wcwl_header_wrap' );
		$I->wait( 1 );
	...
	public function CheckEmailsSentToCustomers( AcceptanceTester $I ) {
	...
	public function ArchiveTabShowsWhenClicked( AcceptanceTester $I ) {
		TestHelperFunctions::VisitWaitlistAdminTab( $I, 'variable', 'archive' );
	...
	public function CanDisableWaitlist( AcceptanceTester $I ) {
		TestHelperFunctions::VisitWaitlistAdminTab( $I, 'variable', 'options' );
		...
		$I->selectOption( 'select#pa_size', 'Large' );
		$I->wait( 1 );

...
class sb_et_woo_li_account_downloads_module extends ET_Builder_Module
{
    function init()
    {
        $this->name = __('Woo Account Downloads', 'et_builder');
        $this->slug = 'et_pb_woo_account_downloads';
    ...
    function shortcode_callback($atts, $content = null, $function_name)
    {
        ...
        if ($content) {
            $output = '<div ' . ($module_id ? 'id="' . esc_attr($module_id) . '"' : '') . ' class="' . $module_class . ' clearfix ' . ($title ? 'has_title':'') . ' et_pb_module et_pb_woo_account et_pb_woo_account_downloads">' . $content . '</div>';
        }
...
new sb_et_woo_li_account_downloads_module();

...
class WC_Memberships_CSV_Export_User_Memberships extends WC_Memberships_Job_Handler {
	...
	public function create_job( $attrs ) {
		...
		if ( '' === $file_name ) {
			throw new SV_WC_Plugin_Exception( esc_html__( "No valid filename given for export file, can't export memberships.", 'woocommerce-memberships' ) );
		}
		...
		if ( $job ) {
			...
			if ( ! wp_mkdir_p( $this->get_file_path() ) ) {
				...
				throw new SV_WC_Plugin_Exception( sprintf( esc_html__( 'Could not create an exports folder in "%s".', 'woocommerce-memberships' ) ) );
			}
			...
			if ( false === $file_handle || ! is_writable( $file_path ) ) {
				...
				throw new SV_WC_Plugin_Exception( sprintf( esc_html__( 'Could not open the export file %s for writing.', 'woocommerce-memberships' ), $file_path ) );
			}
			...
			if ( empty( $headers ) ) {
				...
				throw new SV_WC_Plugin_Exception( esc_html__( 'Could not find CSV headers to write in export file.', 'woocommerce-memberships' ) );
			}
	...
	public function process_job( $job, $items_per_batch = 5 ) {
		...
		if ( ! isset( $job->{$data_key} ) || ! is_array( $job->{$data_key} ) ) {
			throw new SV_WC_Plugin_Exception( esc_html__( 'User memberships to export not set or invalid.', 'woocommerce-memberships' ) );
		}
		...
		if ( ! empty( $user_memberships ) ) {
			...
			foreach ( $user_memberships as $user_membership_id ) {
				...
				try {
					...
					throw new SV_WC_Plugin_Exception( $e->getMessage() );
				}
			...
			if ( ! empty( $columns ) ) {
				...
				if ( ! empty( $user_membership_csv_row_data ) && ! empty( $job->file_path ) ) {
					...
					if ( false === $file_handle || ! is_writable( $job->file_path ) ) {
						...
						/* translators: Placeholders: %s - file name */
						throw new SV_WC_Plugin_Exception( sprintf( esc_html__( 'Could not open the export file %s for writing.', 'woocommerce-memberships' ), $job->file_path ) );
					}
	...
	public function get_user_memberships_ids_for_export( array $export_args ) {
		...
		if ( isset( $export_args['user_membership_ids'] ) ) {
			...
			if ( is_array( $export_args['user_membership_ids'] ) ) {
				...
				throw new SV_WC_Plugin_Exception( esc_html__( 'No User Memberships selected for export.', 'woocommerce-memberships' ) );
			}
		}

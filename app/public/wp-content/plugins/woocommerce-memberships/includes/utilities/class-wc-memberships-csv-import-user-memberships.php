...
class WC_Memberships_CSV_Import_User_Memberships extends WC_Memberships_Job_Handler {
	...
	public function create_job( $attrs ) {
		...
		if ( empty( $attrs['file'] ) || ! is_array( $attrs['file'] ) ) {
			throw new SV_WC_Plugin_Exception( esc_html__( 'You need to provide a valid CSV file to import memberships from.', 'woocommerce-memberships' ) );
		}

		// file upload has an error
		if ( isset( $attrs['file']['error'] ) && is_numeric( $attrs['file']['error'] ) && $attrs['file']['error'] > 0 ) {
			throw new SV_WC_Plugin_Exception( $this->get_file_upload_error_message( $attrs['file']['error'] ) );
		}

		// file is too big
		if ( isset( $attrs['file']['size'] ) && is_numeric( $attrs['file']['size'] ) > wc_let_to_num( ini_get( 'post_max_size' ) ) ) {
			throw new SV_WC_Plugin_Exception( $this->get_file_upload_error_message( 1 ) );
		} else {
		...
		if ( '' === $file_name ) {
			...
			throw new SV_WC_Plugin_Exception( $this->get_file_upload_error_message() );
		} else {
		...
		if ( ! wp_mkdir_p( $imports_dir ) ) {
			throw  new SV_WC_Plugin_Exception( $write_error_message );
		}
		...
		if ( $rows < 1 ) {
			throw new SV_WC_Plugin_Exception( esc_html__( 'The uploaded file seems not to contain user memberships data.', 'woocommerce-memberships' ) );
		}
	 ...
	 * @throws \SV_WC_Plugin_Exception upon error
	 */
	public function process_job( $job, $items_per_batch = 3 ) {
		...
		if ( empty( $job->file_name ) || ! is_string( $job->file_name ) || empty( $job->file_path ) || ! is_string( $job->file_path ) || ! is_readable( $job->file_path ) ) {
			...
			throw new SV_WC_Plugin_Exception( $read_error_message );
		}
		...
		if ( 0 === $headers_columns ) {
			...
			throw new SV_WC_Plugin_Exception( esc_html__( 'Could not find valid CSV headers in uploaded file to import from.', 'woocommerce-memberships' ) );
		}
	...
	public function process_item( $row, $job ) {
		...
		if ( ! $skip_row ) {
			...
			$import_data['membership_expiration'] = ! empty( $row['membership_expiration'] ) ? $row['membership_expiration'] : null;
	...
	private function update_user_membership_meta( WC_Memberships_User_Membership $user_membership, $action, $data, $job ) {
		...
		if ( 'create' === $action || $job->merge_existing_memberships ) {
			...
			if ( $this->is_date( trim( $data['membership_expiration'] ) ) ) {
				$user_membership->set_end_date( $this->parse_date_mysql( trim( $data['membership_expiration'] ), $timezone ) );

	...
	public function process_job( $job, $items_per_batch = 5 ) {
		...
		if ( ! empty( $user_ids ) ) {
			...
			foreach ( $user_ids as $user_id ) {
				...
				if ( $this->process_item( $user_id, $membership_plan ) ) {
		...
		if ( $membership_plan instanceof WC_Memberships_Membership_Plan && is_numeric( $user_id ) ) {
			...
			if ( $user instanceof WP_User && 'publish' === $membership_plan->post->post_status ) {
				...
				switch ( $membership_plan->get_access_method() ) {
					...
					case 'purchase' :
						$granted = $this->grant_access_to_existing_purchases( $user, $membership_plan );
					break;

					// free plan: grant access to users for the simple act of having signed up
					case 'signup' :
						$granted = $this->grant_free_access_to_existing_user( $user, $membership_plan );
					break;
				}
			}
		}
							...
							foreach ( $items as $item ) {
								...
								if ( $product && in_array( $product->get_id(), $product_ids, false ) ) {
									...
									if ( $cumulative_access_allowed ) {
										$existing_membership = wc_memberships_get_user_membership( $user, $membership_plan );
										$grant_access        = ! ( $existing_membership && wc_memberships_has_order_granted_access( $order, array( 'user_membership' => $existing_membership ) ) );
									...
									if ( $really_grant_access ) {
										...
										$user_membership_id = $membership_plan->grant_access_from_purchase( $user->ID, $product->get_id(), $order->get_id() );

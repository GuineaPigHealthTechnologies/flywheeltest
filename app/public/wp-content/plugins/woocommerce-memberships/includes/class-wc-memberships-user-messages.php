...
class WC_Memberships_User_Messages {
	...
	public static function get_message( $message_code, $message_args = array() ) {
		...
		if ( $the_post = ! $the_post instanceof WP_Post ? $post : $the_post ) {
			...
			/* TODO remove traces of legacy key handling in the following block of code inside this if statement by version 1.12.0 {FN 2017-12-18} */
			$use_override_key     = null;
			$message_override_key = null;
			$legacy_use_override  = 'product' !== $the_post->post_type && 'yes' === wc_memberships_get_content_meta( $the_post->ID, '_wc_memberships_use_custom_content_restricted_message' );
			...
			if ( SV_WC_Helper::str_ends_with( $message_code, 'no_products' ) ) {
				...
				if ( $legacy_use_override || 'yes' === wc_memberships_get_content_meta( $the_post->ID, $use_override_key ) ) {
			...
			if ( $legacy_use_override ) {

				// sanity check: has a new message override been using the new meta key yet?
				if ( empty( $message_override ) ) {

					// if not, then use the legacy message override
					$legacy_override = wc_memberships_get_content_meta( $the_post->ID, '_wc_memberships_content_restricted_message' );

					if ( is_string( $legacy_override ) ) {

						$message_override = $legacy_override;

						// if a legacy message override is found, move its contents to the new post meta introduced in v1.9.0
						wc_memberships_set_content_meta( $the_post->ID, SV_WC_Helper::str_ends_with( $message_code, 'no_products' ) ? substr( "_wc_memberships_{$message_code}", 0, -12 ) : "_wc_memberships_{$message_code}", $message_override );
						wc_memberships_delete_content_meta( $the_post->ID, '_wc_memberships_content_restricted_message' );
					}
				}

				// move use custom message flag to new post meta introduced in v1.9.0
				wc_memberships_set_content_meta( $the_post->ID, SV_WC_Helper::str_ends_with( $message_code, 'no_products' ) ? substr( "_wc_memberships_use_custom_{$message_code}", 0, -12 ) : "_wc_memberships_use_custom_{$message_code}", 'yes' );
				wc_memberships_delete_content_meta( $the_post->ID, '_wc_memberships_use_custom_content_restricted_message' );
			}
		}
	...
	public static function get_message_html( $code_shorthand, $args = array() ) {
		...
		$products     = self::get_products_that_grant_access( $the_post, $rule_type, $args );
		...
		$message_code = empty( $products ) && ! in_array( $code_shorthand, array( 'cart_sole_item_discount', 'cart_item_discount', 'cart_items_discount', 'member_login' ), false ) && ! SV_WC_Helper::str_ends_with( $code_shorthand, 'delayed' ) ? $code_shorthand . '_message_no_products' : $code_shorthand . '_message';
		$message      = do_shortcode( self::get_message( $message_code, $args ) );
		$message      = self::handle_deprecated_filters( $message, $message_code, $args );
		// handle merge tags
		$message      = self::parse_message_merge_tags( $message, $args );
		// parse allowed HTML
		...
		if ( 'content' === $args['context'] ) {
			...
			if ( $the_post && ! $the_term && wc_memberships()->get_restrictions_instance()->showing_excerpts() ) {
		 ...
		 * @param array $args array of arguments used to build the message
		 */
		return (string) apply_filters( 'wc_memberships_restricted_message_html', $html_message, $the_post, $message_code, $args );
	}
	...
	public static function parse_message_merge_tags( $message, $args = array() ) {
		...
		if ( ! empty( $products ) ) {
			...
			// ...however if we don't, then set a fallback:
				...
				if ( '' !== $message_code && ! SV_WC_Helper::str_ends_with( $message_code, '_no_products' ) ) {

					unset( $args['products'] );

					$message_code  = SV_WC_Helper::str_ends_with( $message_code, '_message' ) ? "{$message_code}_no_products" : "{$message_code}_message_no_products";
	...
	/**
	 * Returns a list of products that grant access to a piece of content.
	 *
	 * @since 1.9.0
	 *
	 ...
	 * @return array
	 */
	private static function get_products_that_grant_access( $post_id = null, $rule_type = '', $args = array() ) {
		global $post;

		$filtered_products = $unfiltered_products = array();
		 ...
		 * @since 1.4.0
		 *
		 * @param int[] $filtered_products IDs of the products that grant access
		 ...
		 */
		return (array) apply_filters( 'wc_memberships_products_that_grant_access', ! empty( $filtered_products ) ? array_unique( $filtered_products ) : array(), $post_id, $rule_type, $args, $unfiltered_products );
	}

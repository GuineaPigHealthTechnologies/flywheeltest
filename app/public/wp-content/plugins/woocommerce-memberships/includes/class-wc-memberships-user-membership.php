...
class WC_Memberships_User_Membership {
		...
		if ( is_numeric( $end_timestamp ) && (int) $end_timestamp > strtotime( 'today', $now ) ) {
			...
			if ( $end_timestamp > $now ) {
				...
				if ( $days_before_expiry > $now ) {
					...
					wc_schedule_single_action( max( $now + MINUTE_IN_SECONDS, round( ( $now + $end_timestamp ) / 2 ) ), 'wc_memberships_user_membership_expiring_soon', $hook_args, 'woocommerce-memberships' );
				}
			}
		}
	}
	...
	public function schedule_post_expiration_events( $expiration_time ) {
		...
		if ( $this->can_be_renewed() ) {
			wc_schedule_single_action( $this->get_expired_time_after( $expiration_time ), 'wc_memberships_user_membership_renewal_reminder', $hook_args, 'woocommerce-memberships' );
		}
	}

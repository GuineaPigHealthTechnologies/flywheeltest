...
class WooCommerce_Product_Search_Term_Node {
	...
	public function __construct( $term_ids, $taxonomy, $options = null, &$index = null ) {
			...
			foreach ( $term_ids as $term_id ) {
				...
				try {
					if ( key_exists( $term_id, $this->index ) ) {
						...
						$node = new WooCommerce_Product_Search_Term_Node( $term_id, $taxonomy, $options, $this->index );
					}
	...
	public function compare( $a, $b ) {
		$result = 0;
		$term_a = get_term( $a->term_id, $a->taxonomy );
		$term_b = get_term( $b->term_id, $b->taxonomy );
		switch ( $this->orderby ) {
			case 'name' :
			...
			case 'term_order' :
				...
				if ( $term_a instanceof WP_Term ) {
					if ( taxonomy_is_product_attribute( $a->taxonomy ) ) {
				...
				if ( $term_b instanceof WP_Term ) {
					if ( taxonomy_is_product_attribute( $b->taxonomy ) ) {

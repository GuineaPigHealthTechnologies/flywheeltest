...
class WooCommerce_Product_Search_Filter_Tag {
	...
	public static function render( $atts = array(), &$results = null ) {
		...
		$open_inside_output = sprintf( '<div class="tagcloud product-tags product-search-filter-items product-search-filter-tag product-search-filter-%s %s">', esc_attr( $taxonomy ), esc_attr( $list_class ) );
			...
			if ( $spread <= 0 )
				...
				if ( $font_spread < 0 )
					...
					foreach ( $tags_data as $key => $tag_data ) {
						...
						$thumbnail = '';
						if ( $show_thumbnails ) {
							$thumbnail = WooCommerce_Product_Search_Thumbnail::term_thumbnail( $tag, $thumbnail_params );
						}
						$term_name = '';
						if ( $show_names ) {
							$term_name =

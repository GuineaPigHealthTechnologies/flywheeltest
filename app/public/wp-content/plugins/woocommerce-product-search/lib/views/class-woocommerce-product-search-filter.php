...
class WooCommerce_Product_Search_Filter {
	...
	public static function render( $atts = array(), &$results = null ) {
		...
		$output .= sprintf(
			'<form id="%s" class="product-search-form" action="%s" method="get">',
			esc_attr( $form_id ),
			esc_url( $href )
		);
		$output .= sprintf(
		...
		$output .= sprintf(
			'jQuery("#%s").on("ixTermFilter", function(e,term,taxonomy,action) {' .
			'var query = jQuery(this).val();' .
			'switch( action ) {' .
			'case "replace":' .
			'case "add":' .
			...
			'action = "replace";' .
			'}' . 
			'ixwpsf.productFilter(query, %s, jQuery.extend({},%s,{term:term,taxonomy:taxonomy,action:action}));' .
			'} );', 
			esc_attr( $field_id ), 
			$containers, 
			$js_args 

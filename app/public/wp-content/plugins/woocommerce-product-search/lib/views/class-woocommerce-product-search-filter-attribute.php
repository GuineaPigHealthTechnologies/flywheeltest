...
class WooCommerce_Product_Search_Filter_Attribute {
	...
	public static function render( $atts = array(), &$results = null ) {
		...
		$open_inside_output = sprintf( '<ul class="product-attribute product-search-filter-items product-search-filter-attribute product-search-filter-%s %s">', esc_attr( $taxonomy ), esc_attr( $list_class ) );

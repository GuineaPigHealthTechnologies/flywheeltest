...
class WooCommerce_Product_Search_Indexer {
	...
	public function get_processable_count() {
		...
		if (
			...
			$object_type_id = $wpdb->get_var(
				"SELECT object_type_id FROM $object_type_table WHERE object_type = 'product' AND context_table = 'posts' AND context_column IS NULL AND context_key IS NULL"
			);
		...
		if ( $product ) {
			...
			if ( $this->raw ) {
				$context_columns = array(
					'post_title'   => $this->filter( $product->post_title . $titles, 'post_title' ),
					'post_excerpt' => $this->filter( $product->post_excerpt . $descriptions, 'post_excerpt' ),
					'post_content' => $this->filter( $product->post_content, 'post_content' )
				);
			} else {
				$context_columns = array(
					'post_id'      => $this->filter( $product->get_id() . $ids, 'post_id' ),
					'post_title'   => $this->filter( $product->get_title() . $titles, 'post_title' ),
					'post_excerpt' => $this->filter( wc_format_content( $product->get_short_description() ), 'post_excerpt' ),
					'post_content' => $this->filter( wc_format_content( $product->get_description() ) . $descriptions, 'post_content' ),
					'sku'          => $this->filter( $product->get_sku() . $skus, 'sku' ),
					'tag'          => $this->filter( wc_get_product_tag_list( $post_id, ' ' ), 'tag' ),
					'category'     => $this->filter( wc_get_product_category_list( $post_id, ' ' ), 'category' )
				);
				...
				if ( !empty( $attribute_taxonomies ) ) {
					foreach ( $attribute_taxonomies as $attribute ) {
						...
						if ( !empty( $term_list ) && is_string( $term_list ) ) { 
							...
							$context_columns[$attribute->attribute_name] = $this->filter( $term_list, $attribute->attribute_name );
						}
					}
				}
			}
			...
			foreach( $context_columns as $context_column => $content ) {
				switch( $context_column ) {
					case 'category' :
						$object_type_id = $this->get_object_type_id( 'product', 'category', 'term_taxonomy', 'taxonomy', 'product_cat' );
						break;
					case 'tag' :
						$object_type_id = $this->get_object_type_id( 'product', 'tag', 'term_taxonomy', 'taxonomy', 'product_tag' );
						break;
					case 'sku' :
						$object_type_id = $this->get_object_type_id( 'product', 'sku', 'postmeta', 'meta_key', '_sku' );
						break;
					default :
						if ( !in_array( $context_column, $attribute_names ) ) {
							$object_type_id = $this->get_object_type_id( 'product', 'product', 'posts', $context_column );
						} else {
							$object_type_id = $this->get_object_type_id( 'product', $context_column, 'term_taxonomy', 'taxonomy', 'pa_' .$context_column );
							if ( $object_type_id === null ) {
								$object_type_id = $this->get_or_add_object_type( 'product', $context_column, 'term_taxonomy', 'taxonomy', 'pa_' .$context_column );
							}
						}
				}
				if ( $object_type_id !== null ) {
			...
			$object_type_id = $wpdb->get_var( "SELECT object_type_id FROM $object_type_table WHERE object_type = 'product' AND context_table = 'posts' AND context_column IS NULL AND context_key IS NULL" );
	...
	public function filter( $content, $context = '' ) {

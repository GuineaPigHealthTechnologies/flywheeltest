...
class WooCommerce_Product_Search_Admin {
		...
		if ( $screen && function_exists( 'wc_get_screen_ids' ) && in_array( $screen->id, wc_get_screen_ids() ) ) {
			...
			$content .= wp_kses(
				sprintf(
					...
					esc_url( 'http://docs.woothemes.com/document/woocommerce-product-search/' )
				),
				...
				switch ( $current_section ) {
					...
					case self::SECTION_GENERAL :
						...
						if ( $options[WooCommerce_Product_Search::AUTO_REPLACE_FORM] ) {
							...
							$field_names = array( 'query_title', 'excerpt', 'content', 'categories', 'attributes', 'tags', 'sku', 'order', 'order_by', 'limit', 'show_more', 'show_clear', 'category_results', 'product_thumbnails', 'show_description', 'show_price', 'show_add_to_cart', 'delay', 'characters', 'placeholder', 'dynamic_focus', 'floating', 'inhibit_enter', 'submit_button', 'submit_button_label', 'navigable', 'no_results', 'auto_adjust', 'wpml' );
							$new_instance = array();
					...
					case self::SECTION_WEIGHTS :
						...
						$options[WooCommerce_Product_Search::WEIGHT_TITLE]      = !empty( $_POST[WooCommerce_Product_Search::WEIGHT_TITLE] ) ? intval( $_POST[WooCommerce_Product_Search::WEIGHT_TITLE] ) : WooCommerce_Product_Search::WEIGHT_TITLE_DEFAULT;
						$options[WooCommerce_Product_Search::WEIGHT_EXCERPT]    = !empty( $_POST[WooCommerce_Product_Search::WEIGHT_EXCERPT] ) ? intval( $_POST[WooCommerce_Product_Search::WEIGHT_EXCERPT] ) : WooCommerce_Product_Search::WEIGHT_EXCERPT_DEFAULT;
						$options[WooCommerce_Product_Search::WEIGHT_CONTENT]    = !empty( $_POST[WooCommerce_Product_Search::WEIGHT_CONTENT] ) ? intval( $_POST[WooCommerce_Product_Search::WEIGHT_CONTENT] ) : WooCommerce_Product_Search::WEIGHT_CONTENT_DEFAULT;
						$options[WooCommerce_Product_Search::WEIGHT_TAGS]       = !empty( $_POST[WooCommerce_Product_Search::WEIGHT_TAGS] ) ? intval( $_POST[WooCommerce_Product_Search::WEIGHT_TAGS] ) : WooCommerce_Product_Search::WEIGHT_TAGS_DEFAULT;
						$options[WooCommerce_Product_Search::WEIGHT_CATEGORIES] = !empty( $_POST[WooCommerce_Product_Search::WEIGHT_CATEGORIES] ) ? intval( $_POST[WooCommerce_Product_Search::WEIGHT_CATEGORIES] ) : WooCommerce_Product_Search::WEIGHT_CATEGORIES_DEFAULT;
						$options[WooCommerce_Product_Search::WEIGHT_ATTRIBUTES] = !empty( $_POST[WooCommerce_Product_Search::WEIGHT_ATTRIBUTES] ) ? intval( $_POST[WooCommerce_Product_Search::WEIGHT_ATTRIBUTES] ) : WooCommerce_Product_Search::WEIGHT_ATTRIBUTES_DEFAULT;
						$options[WooCommerce_Product_Search::WEIGHT_SKU]        = !empty( $_POST[WooCommerce_Product_Search::WEIGHT_SKU] ) ? intval( $_POST[WooCommerce_Product_Search::WEIGHT_SKU] ) : WooCommerce_Product_Search::WEIGHT_SKU_DEFAULT;
						break;
	...
	public static function woocommerce_product_search() {
		...
		switch ( $current_section ) {
			...
			case self::SECTION_GENERAL :
				...
				echo '<p>';
				echo '<label>';
				printf( '<input name="%s" type="checkbox" %s />', WooCommerce_Product_Search::AUTO_REPLACE_ADMIN, $auto_replace_admin ? ' checked="checked" ' : '' );
				echo ' ';
	...
	public static function plugin_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ) {
		if ( $plugin_file == plugin_basename( WOO_PS_FILE ) ) {
			$plugin_meta[] = '<a style="font-weight:bold" href="http://docs.woothemes.com/document/woocommerce-product-search/">' . esc_html__( 'Documentation', 'woocommerce-product-search' ) . '</a>';

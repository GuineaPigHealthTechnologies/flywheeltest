...
function woocommerce_wishlist_handle_add_to_wishlist_action() {
	...
	if ( $added_to_wishlist && $wishlist_id != 'session' ) {
		...
		if ( $added_to_wishlist ) {
			...
			$action  = '<a class="wishlist-message-dismiss" href="' . WC_Wishlists_Plugin::nonce_url( 'clear-session-items', add_query_arg( array( 'wlaction' => 'clear-session-items' ) ) ) . '">' . __( 'Cancel', 'wc_wishlist' ) . '</a>';
...
function woocommerce_wishlists_logout() {
	if ( isset( $_COOKIE['wc_wishlists_user'] ) ) {
		unset( $_COOKIE['wc_wishlists_user'] );
		setcookie( 'wc_wishlists_user', null, - 1, '/' );
	}
}

...
abstract class WC_Zapier_Trigger_Order extends WC_Zapier_Trigger {
	...
	protected function get_sample_data() {
		...
		$order = array(
		   ...
		   'date' => '2013-08-22T06:54:00+08:00',
		   'total' => '176.00',
		   'subtotal' => '176.00',
		   'currency' => 'USD',
		   'currency_symbol' => '$',
		   ...
		   'notes' => array(
				 array(
						...
						'date' => '2013-08-23T01:24:55+08:00',
						'author' => 'WooCommerce',
	...
	public function assemble_data( $args, $action_name ) {
		...
		// Order Details
		$order->id              = is_callable( array( $this->wc_order, 'get_id' ) ) ? $this->wc_order->get_id() : $this->wc_order->id; // Order ID (integer) // @codingStandardsIgnoreLine
		...
		foreach ( $this->wc_order->get_items() as $line_item_data ) {
			...
			// Line Item Data.
			if ( is_callable( array( $this->wc_order, 'get_item_total' ) ) ) { // WC 3.0+ only
	...
	protected function data_sent_to_feed( WC_Zapier_Feed $feed, $result, $action_name, $arguments, $num_attempts = 0 ) {
		...
		WC_Zapier()->log( $note, is_callable( array( $this->wc_order, 'get_id') ) ? $this->wc_order->get_id() : $this->wc_order->id ); // @codingStandardsIgnoreLine

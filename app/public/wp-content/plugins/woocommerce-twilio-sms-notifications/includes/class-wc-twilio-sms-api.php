 ...
 *
 * @package     WC-Twilio-SMS-Notifications/API
 * @author      SkyVerge
...
class WC_Twilio_SMS_API {
	...
	public function send( $to, $body, $country_code = null ) {
		...
		if ( SV_WC_Helper::str_starts_with( $to, '+' ) ) {
			foreach( $this->get_country_codes() as $code => $prefix ) {

				if ( SV_WC_Helper::str_starts_with( $to, '+' . $prefix ) ) {
					$country_code = $code;
					break;
				}
			}
		...
		$body = SV_WC_Helper::str_truncate( $body, 160 );

		// Use the Alphanumeric Sender ID if supported by the recieving country
		if ( ! empty( $this->asid ) && in_array( $country_code, $this->get_asid_country_codes() ) ) {
			$from = $this->asid;
		} else {

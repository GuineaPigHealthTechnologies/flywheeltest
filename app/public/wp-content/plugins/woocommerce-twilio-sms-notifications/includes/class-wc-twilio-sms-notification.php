...
class WC_Twilio_SMS_Notification  {
	...
	private function google_shorten_url( $url, $api_key ) {
		...
		$error_message = __( 'URL Shortener error', 'woocommerce-twilio-sms-notifications' );
	...
	private function replace_message_variables( $message ) {
		...
		$replacements = array(
			...
			'%billing_first%'   => SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_first_name' ),
			'%billing_last%'    => SV_WC_Order_Compatibility::get_prop( $this->order, 'billing_last_name' ),
		);

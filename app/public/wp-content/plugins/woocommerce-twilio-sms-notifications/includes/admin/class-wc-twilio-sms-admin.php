...
class WC_Twilio_SMS_Admin {
	...
	public function add_settings_tab( $settings_tabs ) {
		...
		foreach ( $settings_tabs as $tab_id => $tab_title ) {
			...
			if ( 'email' === $tab_id ) {
				$new_settings_tabs[ $this->tab_id ] = __( 'SMS', 'woocommerce-twilio-sms-notifications' );
			}
		}
	...
	public static function get_settings() {
		...
		$settings = array(
			...
			array(
				...
				'desc'    => __( 'Enable to automatically shorten links in SMS messages via the Google URL Shortener.', 'woocommerce-twilio-sms-notifications' ),
				'default' => 'yes',
				'type'    => 'checkbox',
			),
			...
			array(
				...
				'name'  => __( 'Google API Key', 'woocommerce-twilio-sms-notifications' ),
				'type'  => 'text',
			),
		...
		$settings = array_merge( $settings, array(
			...
			array(
				...
				'desc_tip' => __( 'Enter the response message to be sent. Remember that SMS messages are limited to 160 characters. Leave blank to store the inbound SMS but disable replies.', 'woocommerce-twilio-sms-notifications' ),
			...
			array(
				...
				'desc_tip' => __( 'Enter the test message to be sent. Remember that SMS messages are limited to 160 characters.', 'woocommerce-twilio-sms-notifications' ),
				'type'     => 'textarea',
				'css'      => 'min-width: 500px;',
			),

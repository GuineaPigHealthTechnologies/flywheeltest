...
class WC_CP_Compatibility {
	...
	public function module_includes() {
		...
		if ( class_exists( 'WC_Bundles' ) && function_exists( 'WC_PB' ) && version_compare( str_replace( '-dev', '', WC_PB()->version ), $this->required[ 'pb' ] ) >= 0 ) {
	...
	public function check_required_versions() {
		...
		if ( ! empty( $woocommerce_bundles ) ) {
			...
			if ( version_compare( str_replace( '-dev', '', $woocommerce_bundles->version ), $required_version ) < 0 ) {

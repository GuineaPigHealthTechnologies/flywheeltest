...
class WC_Product_Reviews_Pro_Admin {
	...
	public function contribution_status_links( array $status_links ) {
		...
		return SV_WC_Helper::array_insert_after( $status_links, 'spam', array( 'is_flagged' => $flagged_link ) );
	}
	...
	public function load_styles_scripts( $hook_suffix ) {
		...
		if ( 'comment.php' === $hook_suffix || 'product_page_reviews' === $hook_suffix || SV_WC_Plugin_Compatibility::normalize_wc_screen_id( 'reviews' ) === $hook_suffix ) {
		...
		// settings
		} elseif ( isset( $_REQUEST['tab'] ) && 'products' === $_REQUEST['tab'] && SV_WC_Plugin_Compatibility::normalize_wc_screen_id() === $hook_suffix ) {
	...
	public function modify_contributions_query($comment_query ) {
		...
		if ( $screen instanceof WP_Screen && ( 'product_page_reviews' === $screen->id || SV_WC_Plugin_Compatibility::normalize_wc_screen_id( 'reviews' ) === $screen->id ) ) {
	...
	public function __call( $method, $args ) {
		...
		switch ( $method ) {
			...
			case 'get_contribution_settings' :

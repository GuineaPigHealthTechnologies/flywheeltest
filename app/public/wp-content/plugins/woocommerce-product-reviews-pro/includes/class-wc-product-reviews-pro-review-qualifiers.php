...
class WC_Product_Reviews_Pro_Review_Qualifiers {
	...
	public function edit_qualifier_options_field( $term ) {
		...
		$options = get_woocommerce_term_meta( $term->term_id, 'options' );
	...
	public function add_qualifier_options_field( $term ) {
		...
		// TODO when requiring WP 4.4 use get_term_meta instead, since 4.4 has its own table and WC itself will deprecate this {FN 2016-05-27}
		$options = is_object( $term ) ? get_woocommerce_term_meta( $term->term_id, 'options' ) : '';
	...
	public function save_qualifier_options( $term_id ) {
		...
		if ( $term_id && isset( $_POST['options'] ) ) {
			...
			// TODO when requiring WP 4.4 use update_term_meta instead, since 4.4 has its own table and WC itself will deprecate this {FN 2016-05-27}
			update_woocommerce_term_meta( $term_id, 'options', $sanitized_options );
		}
	}
	...
	public function delete_qualifier_options( $term_id ) {
		...
		// TODO when requiring WP 4.4 use delete_term_meta instead, since 4.4 has its own table and WC itself will deprecate this {FN 2016-05-27}

...
class WooCommerce_Group_Coupon_Roles {
	...
	public static function woocommerce_coupon_data_panels() {
		...
		echo '<p>';
		echo __( '<strong>Roles</strong> - limit the coupon to roles', 'woocommerce-group-coupons' );
		echo '</p>';
		...
		if ( count( $roles ) > 0 ) {
			...
			if ( !defined( 'WC_VERSION' ) || ( version_compare( WC_VERSION , '2.3.0' ) < 0 ) ) {
				printf( '<select class="_groupon_roles multiselect" name="_groupon_roles[]" multiple="multiple" title="%s">', esc_attr( __( 'No restriction', 'woocommerce-group-coupons' ) ) );
			} else {
				echo '<label for="_groupon_roles">';
				_e( 'Roles', 'woocommerce-group-coupons' );
				echo '</label>';
				echo '<select id="_groupon_roles" name="_groupon_roles[]" style="width: 50%;"  class="wc-enhanced-select" multiple="multiple" data-placeholder="' . __( 'No roles', 'woocommerce-group-coupons' ) . '">';
			}
			...
			echo '<p>';
			__( 'There are no roles available.', 'woocommerce-group-coupons' );
			echo '</p>';
		}

		echo '<p>';
		echo __( 'Only users who have one of the selected roles will be allowed to use the coupon.', 'woocommerce-group-coupons' );
		echo ' ';
		echo __( 'If no role is selected, the coupon is not restricted to any roles.', 'woocommerce-group-coupons' );
		echo '</p>';
		echo '</div>';

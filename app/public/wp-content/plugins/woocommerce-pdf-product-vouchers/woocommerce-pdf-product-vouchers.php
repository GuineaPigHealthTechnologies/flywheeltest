 ...
 * Plugin URI: http://www.woocommerce.com/products/pdf-product-vouchers/
 ...
 *
 * Woo: 201963:76aaaee5ebc9fdab1847c6f3caf295b3
...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.9.0', __( 'WooCommerce PDF Product Vouchers', 'woocommerce-pdf-product-vouchers' ), __FILE__, 'init_woocommerce_pdf_product_vouchers', array(
	'minimum_wc_version'   => '2.6.14',
	'minimum_wp_version'   => '4.7',
...
class WC_PDF_Product_Vouchers extends SV_WC_Plugin {
	...
	public function includes() {
		...
		// Background generator must be loaded all the time, because
		...
		$this->background_generator = $this->load_class( '/includes/class-wc-pdf-product-vouchers-background-generator.php', 'WC_PDF_Product_Vouchers_Background_Generator' );
		...
		$this->redemption_handler = $this->load_class( '/includes/class-wc-pdf-product-vouchers-redemption-handler.php', 'WC_PDF_Product_Vouchers_Redemption_Handler' );
	...
	/**
	 * Returns product class instance
	 *
	 * @since 2.6.0
	...
	/**
	 * Returns my account class instance
	 *
	 * @since 2.6.0
	...
	/**
	 * Returns voucher handler class instance
	 *
	 * @since 2.6.0
	...
	/**
	 * Returns background generator class instance
	 *
	 * @since 3.2.0
	...
	public function add_email_classes( $email_classes ) {
		...
		$email_classes['WC_PDF_Product_Vouchers_Email_Voucher_Purchaser'] = $this->load_class( '/includes/emails/class-wc-pdf-product-vouchers-email-voucher-purchaser.php', 'WC_PDF_Product_Vouchers_Email_Voucher_Purchaser' );
	...
	public static function erase_vouchers_personal_data( $email_address ) {
		...
		if ( is_string( $email_address ) && SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.4.0' ) ) {
	...
	private function add_php_version_notice() {
		...
		if ( version_compare( phpversion(), '5.3.0', '<' ) ) {
			...
			$message = sprintf(
				...
				__( '%1$s requires PHP version at least %2$s to function. Your server is currently using version %3$s. Contact your host or server administrator to upgrade PHP.', 'woocommerce-pdf-product-vouchers' ),

 ...
 * @since 3.0.0
 */
class WC_Voucher_Template extends WC_Voucher_Base {
	...
	public static function get_voucher_fields() {
		...
		$fields = array(
			...
			'voucher_number' => array(
				'data_type' => 'property',
				'label'     => __( 'Voucher Number', 'woocommerce-pdf-product-vouchers' ),
			),
		...
		if ( 'user_input' === $data_type )	{
			...
			$controls = array_merge( $controls, array(
				...
				'is_required' => array(
					...
					'description' => __( 'Make this field required in order to add a voucher product to the cart', 'woocommerce-pdf-product-vouchers' ),
				),
			) );
		}
	...
	public static function get_voucher_field_controls( $field_id, $attrs ) {
		...
		elseif ( 'expiration_date' === $field_id ) {
			$controls['days_to_expiry'] = array(
				...
				'description' => __( 'Optional number of days after purchase until the voucher expires', 'woocommerce-pdf-product-vouchers' ),
				'input_attrs' => array(
					'min'         => 0,
					'step'        => 1,
		...
		elseif ( 'product_quantity' === $field_id ) {
			$controls['amount'] = array(
				...
				'description' => __( 'Number of products/tickets/passess this voucher is eligible for', 'woocommerce-pdf-product-vouchers' ),
				'input_attrs' => array(
					'min'         => 1,
					'step'        => 1,
				),
		...
		elseif ( 'logo' === $field_id ) {
			$controls = array(
				'image_id' => array(
					'type'          => 'image',
					'label'         => __( 'Logo', 'woocommerce-pdf-product-vouchers' ),
					'description'   => __( 'Optional image that can be displayed as a logo on the voucher.', 'woocommerce-pdf-product-vouchers' ),
					'button_labels' => array(
						...
						'frame_title'  => __( 'Select voucher logo', 'woocommerce-pdf-product-vouchers' ),
	 ...
	 * @return array
	 */
	private static function get_voucher_font_family_options( $include_default = true ) {

		/**
		 * Filters vucher font family options
		 *

...
class WC_PDF_Product_Vouchers_Handler {
	...
	public function voucher_emails_attachments( $attachments, $email_type, $object ) {
		...
		if ( in_array( $email_type, array( 'customer_completed_order', 'customer_invoice', 'wc_pdf_product_vouchers_voucher_purchaser' ) ) ||
			...
			$vouchers = 'wc_pdf_product_vouchers_voucher_purchaser' === $email_type ? $object['vouchers'] : WC_PDF_Product_Vouchers_Order::get_vouchers( $object );
	...
	public function generate_voucher_pdf( $voucher ) {
		...
		try {
			...
			$voucher->get_order()->add_order_note( sprintf( __( 'PDF Product Vouchers: unable to generate PDF for voucher %1$s: %2$s', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number(), $exception->getMessage() ) );
		}
	}
		...
		foreach ( $order->get_refunds() as $refund ) {
			...
			foreach ( $refund->get_items() as $refunded_item ) {
				...
				if ( $item ) {
					...
					foreach ( WC_PDF_Product_Vouchers_Order::get_order_item_vouchers( $item ) as $voucher ) {
						...
						// void the voucher
						$voucher->void( array(
							'reason' => __( 'Voucher was refunded.', 'woocommerce-pdf-product-vouchers' )
						) );
					}
				}
			}
	...
	public function order_item_meta_start( $item_id, $item, WC_Order $order ) {
		...
		printf( '<strong>%s</strong> %s', esc_html( _n( 'Voucher:', 'Vouchers:', count( $links ), 'woocommerce-pdf-product-vouchers' ) ), implode( ', ', $links ) );
		...
		if ( $generating > 0 && ! $is_admin && ! $this->rendering_email ) {
			...
			$message  = esc_html( _n( "We're preparing your voucher - it will be available for download soon!", "We're preparing your vouchers - they'll be available for download soon!", $generating, 'woocommerce-pdf-product-vouchers' ) );
	...
	public function transition_post_status( $new_status, $old_status, WP_Post $post ) {
		...
		$status_note   = sprintf( __( 'Voucher status changed from %1$s to %2$s.', 'woocommerce-pdf-product-vouchers' ), wc_pdf_product_vouchers_get_voucher_status_name( $old_status ), wc_pdf_product_vouchers_get_voucher_status_name( $new_status ) );
	...
	public function get_voucher_statuses() {
		...
		$statuses = array(
			...
			// redeemed/used.
			'wcpdf-pending'  => array(
				'label'       => _x( 'Pending', 'Voucher Status', 'woocommerce-pdf-product-vouchers' ),
			...
			'wcpdf-active'   => array(
				'label'       => _x( 'Active', 'Voucher Status', 'woocommerce-pdf-product-vouchers' ),

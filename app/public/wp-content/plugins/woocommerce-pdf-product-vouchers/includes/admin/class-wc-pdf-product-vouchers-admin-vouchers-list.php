 ...
 * @since 3.0.0
 */
class WC_PDF_Product_Vouchers_Admin_Vouchers_List {
	...
	public function __construct() {
		...
		add_filter( 'manage_edit-wc_voucher_sortable_columns', array( $this, 'edit_voucher_sortable_columns' ) );

		add_action( 'manage_wc_voucher_posts_custom_column', array( $this, 'custom_voucher_columns' ), 2 );

		add_filter( 'display_post_states', array( $this, 'edit_voucher_post_states' ), 10, 2 );

		add_filter( 'post_row_actions', array( $this, 'edit_voucher_row_actions' ), 10, 2 );
	...
	/**
	 * Removes the bulk edit action for vouchers, it really isn't useful
	 *
	 * @since 3.0.0
	...
	public function custom_voucher_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'remaining_value':
				...
				$display_value     = wc_price( $voucher->get_voucher_value_for_display() );
				$display_remaining = wc_price( $voucher->get_remaining_value_for_display() );
				...
				echo sprintf( esc_html__( '%1$s of %2$s', 'woocommerce-pdf-product-vouchers' ), $display_remaining, $display_value );

				// show how may uses the voucher still has
				if ( 'single' === $voucher->get_voucher_type() && $voucher->get_product_quantity() > 1 ) {
					echo '<br><small class="remaining-quantity">' . sprintf( esc_html__( '%1$s of %2$s uses', 'woocommerce-pdf-product-vouchers' ), $voucher->get_remaining_value() / $voucher->get_product_price(), $voucher->get_product_quantity() ) . '</small>';
				}
			...
			case 'voucher_actions':
				?><p>
					...
					<input type="hidden" class="voucher-value" value="<?php echo $voucher->get_voucher_value(); ?>" />
					<input type="hidden" class="voucher-tax" value="<?php echo $voucher->get_voucher_tax(); ?>" />
					<input type="hidden" class="voucher-type" value="<?php echo $voucher->get_voucher_type(); ?>" />
					<input type="hidden" class="voucher-product-price" value="<?php echo $voucher->get_product_price(); ?>" />
					<input type="hidden" class="voucher-product-price-for-display" value="<?php echo $voucher->get_product_price_for_display(); ?>" />
					<input type="hidden" class="voucher-remaining-value" value="<?php echo $voucher->get_remaining_value(); ?>" />
					<input type="hidden" class="voucher-remaining-value-incl-tax" value="<?php echo $voucher->get_remaining_value_incl_tax(); ?>" />
					<input type="hidden" class="voucher-remaining-value-for-display" value="<?php echo $voucher->get_remaining_value_for_display(); ?>" />

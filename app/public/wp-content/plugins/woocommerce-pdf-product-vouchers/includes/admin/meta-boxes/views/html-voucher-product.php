		...
		<td class="actions" width="1%">
			<?php if ( $voucher->is_editable() ) : ?>
				...
				<?php elseif ( $voucher->get_order_id() ) : ?>
					<?php echo wc_help_tip( __( 'This voucher was purchased in an order, so the product cannot be changed.', 'woocommerce-pdf-product-vouchers' ) ); ?>
				<?php elseif ( $voucher->has_redemptions() ) : ?>
					<?php echo wc_help_tip( __( 'This voucher has redemptions, so the product can no longer be changed.', 'woocommerce-pdf-product-vouchers' ) ); ?>
				<?php endif; ?>
			<?php endif; ?>
		</td>

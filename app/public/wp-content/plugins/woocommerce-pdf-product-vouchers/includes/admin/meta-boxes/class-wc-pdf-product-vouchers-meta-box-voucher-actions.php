...
class WC_PDF_Product_Vouchers_Meta_Box_Voucher_Actions {
	...
	public function add_meta_box() {
		add_meta_box( 'wc-pdf-product-vouchers-voucher-actions', __( 'Voucher Actions', 'woocommerce-pdf-product-vouchers' ), array( $this, 'output' ), 'wc_voucher', 'side', 'high' );
	}
	...
	public function output() {
		...
		<ul class="voucher_actions submitbox">
			...
			<li class="wide">
				...
				<input type="submit" class="button save_voucher button-primary tips" name="save" value="<?php esc_html_e( 'Save Voucher', 'woocommerce-pdf-product-vouchers' ); ?>" data-tip="<?php esc_html_e( 'Save/update the voucher', 'woocommerce-pdf-product-vouchers' ); ?>" />
			</li>
		...
		if ( ! empty( $_POST['wc_voucher_action'] ) ) {
			...
			if ( strstr( $action, 'send_email_' ) ) {
				...
				try {
					...
					wc_pdf_product_vouchers()->get_message_handler()->add_error( sprintf( __( 'Could not generate voucher PDF: %s', 'woocommerce-pdf-product-vouchers' ), $e->getMessage() ) );
				}

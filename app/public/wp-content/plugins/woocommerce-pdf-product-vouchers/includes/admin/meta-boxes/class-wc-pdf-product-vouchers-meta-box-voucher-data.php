 ...
 * @since 3.0.0
 */
class WC_PDF_Product_Vouchers_Meta_Box_Voucher_Data {
	...
	public function add_meta_box() {
		add_meta_box( 'wc-pdf-product-vouchers-voucher-data', __( 'Voucher Data', 'woocommerce-pdf-product-vouchers' ), array( $this, 'output' ), 'wc_voucher', 'normal', 'high' );
	}
	...
	public function output() {
		...
		include( wc_pdf_product_vouchers()->get_plugin_path() . '/includes/admin/meta-boxes/views/html-voucher-data.php' );
	}
	...
	public function save( $post_id, WP_Post $post ) {
		...
		$user_input_fields = array_keys( WC_Voucher_Template::get_voucher_user_input_fields() );

		foreach ( $user_input_fields as $field ) {
			if ( isset( $_POST[ '_' . $field ] ) ) {
				update_post_meta( $post->ID, '_' . $field, wc_clean( $_POST[ '_' . $field ] ) );
			}
		}
		...
		if ( $customer_changed && $voucher->is_editable() && ! $voucher->has_redemptions() && ! $voucher->get_order_id() ) {
			...
			if ( $new_tax != $voucher->get_product_tax() && 'calculate_product_tax' != $action ) {
				wc_pdf_product_vouchers()->get_message_handler()->add_error( __( 'Voucher customer has changed and is taxed differently from previous customer. You may need to recalculate voucher taxes.', 'woocommerce-pdf-product-vouchers' ) );

 ...
 *
 * In 3.0.0 renamed from \WC_PDF_Product_Vouchers_Admin_Voucher_List_Table
 * to \WC_PDF_Product_Vouchers_Admin_Voucher_Templates_List.
 *
 * @since 1.2.0
 */
class WC_PDF_Product_Vouchers_Admin_Voucher_Templates_List {
	...
	public function edit_voucher_template_row_actions( $actions, $post ) {
		...
		if ( 'wc_voucher_template' === $post->post_type ) {
			...
			$duplicate_link_open  = '<a href="' . wp_nonce_url( admin_url( 'edit.php?post_type=wc_voucher_template&action=duplicate_voucher_template&amp;post=' . $post->ID ), 'wc-pdf-product-vouchers-duplicate-voucher-template_' . $post->ID ) . '" title="' . __( 'Make a duplicate from this voucher template', 'woocommerce-pdf-product-vouchers' ) . '" rel="permalink">';
			$duplicate_link_close = '</a>';

			// add duplicate plan action
			$actions['duplicate'] = $duplicate_link_open . esc_html_x( 'Duplicate', 'Duplicate a Voucher Template', 'woocommerce-pdf-product-vouchers' ) . $duplicate_link_close;
	...
	public function duplicate_voucher_template() {
		...
		if ( $post instanceof WP_Post ) {
			...
			wc_pdf_product_vouchers()->get_message_handler()->add_message( __( 'Voucher template copied.', 'woocommerce-pdf-product-vouchers' ) );
			...
			wp_die( __( 'Voucher template creation failed, could not find original template:', 'woocommerce-pdf-product-vouchers' ) . ' ' . $id );
		}
	}

...
class WC_PDF_Product_Vouchers_Admin_Vouchers {
	...
	public function default_voucher_parent( $post_parent, $post_id, $new_postarr, $postarr ) {
		...
		if ( 'post-new.php' === $pagenow && 'auto-draft' === $new_postarr['post_status'] && 'wc_voucher' === $new_postarr['post_type'] ) {
			...
			if ( ! $product_id || ! $product ) {
				...
				wc_pdf_product_vouchers()->get_message_handler()->add_error( __( 'Please select a product to add a voucher for.', 'woocommerce-pdf-product-vouchers' ) );
			...
			if ( ! $voucher_template_id || ! $voucher_template ) {
				...
				wc_pdf_product_vouchers()->get_message_handler()->add_error( __( 'Please select a product with a voucher template to add a voucher.', 'woocommerce-pdf-product-vouchers' ) );
				wp_redirect( wp_get_referer() );
				exit;
			}
	...
	public function load_voucher() {
		...
		if ( 'post-new.php' === $pagenow ) {
			...
			update_post_meta( $post->ID, '_product_price', SV_WC_Product_Compatibility::wc_get_price_excluding_tax( $product, 1, $product->get_regular_price() ) );
			...
			// set currency
			update_post_meta( $post->ID, '_voucher_currency', get_woocommerce_currency() );
		}

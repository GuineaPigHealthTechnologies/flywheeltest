...
class WC_PDF_Product_Vouchers_Customizer {
	...
	public function add_customizer_settings( $wp_customize ) {
		...
		$wp_customize->add_setting( new WC_PDF_Product_Vouchers_Voucher_Template_Setting( $wp_customize, 'wc_voucher_template_post_title', array(
		...
		$wp_customize->add_setting( new WC_PDF_Product_Vouchers_Voucher_Template_Setting( $wp_customize, 'wc_voucher_template_allow_online_redemptions' ) );
	...
	public function add_customizer_controls( $wp_customize ) {
		...
		$wp_customize->add_control( 'wc_voucher_template_allow_online_redemptions', array(
			...
			'section' => 'wc_voucher_template_general',
			'description' => SV_WC_Plugin_Compatibility::is_wc_version_lt( '3.2' ) ? '<span class="notice notice-warning">' . __( 'Requires WooCommerce at least 3.2', 'woocommerce-pdf-product-vouchers' ) . '</span>' : ''
		) );
	...
	public function customizer_controls_scripts() {
		...
		wp_localize_script( 'woocommerce-pdf-product-vouchers-customizer-control-scripts', 'woocommerce_vouchers_admin_customizer', array(
			...
			'wc_gte_3_2'             => SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.2' )
		) );
	...
	public function validate_voucher_template_number_pos( $validity, $value, $setting ) {
		...
		if ( ! $value && $this->get_setting_value( $online_redemptions, $post_values ) ) {
			$validity->add( 'required', __( 'Voucher number must be added to the template when using online redemptions.', 'woocommerce-pdf-product-vouchers' ) );
		}

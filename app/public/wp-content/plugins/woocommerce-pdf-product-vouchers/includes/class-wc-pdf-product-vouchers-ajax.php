...
class WC_PDF_Product_Vouchers_AJAX {
	...
	public function update_voucher_product() {
		...
		if ( $voucher_id > 0 && $product_id > 0 ) {
			...
			if ( ! $voucher ) {
				wp_send_json_error( array( 'message' => __( 'Invalid voucher', 'woocommerce-pdf-product-vouchers' ) ) );;
			}

			if ( ! $voucher->is_editable() || $voucher->has_redemptions() ) {
				wp_send_json_error( array( 'message' => __( 'Could not change voucher product: voucher is not editable or has redemptions', 'woocommerce-pdf-product-vouchers' ) ) );;
			}
			...
			if ( $template_changed ) {
				...
				try {
					...
					wc_pdf_product_vouchers()->log( sprintf( __( 'Could not generate voucher PDF: %s', 'woocommerce-pdf-product-vouchers' ), $e->getMessage() ) );
				}
	...
	public function calculate_voucher_product_tax() {
		...
		if ( $voucher_id > 0 ) {
			...
			if ( ! $voucher ) {
				...
				wp_send_json_error( array( 'message' => __( 'Could not recalculate voucher product tax: voucher is not editable or has redemptions', 'woocommerce-pdf-product-vouchers' ) ) );
			}
	...
	public function save_voucher_redemptions() {
		...
		if ( $voucher_id > 0 ) {
			...
			if ( ! $voucher->is_editable() ) {
				wp_send_json_error( array( 'message' => __( 'Voucher is not editable', 'woocommerce-pdf-product-vouchers' ) ) );
			}
		...
		if ( $voucher_id > 0 ) {
			...
			try {
				...
				wp_send_json_error( array(
					...
					'message' => sprintf( __( 'Could not redeem voucher %1$s: %2$s', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number(), $e->getMessage() ),
				) );
			}
		}
	...
	public function restore_voucher() {
		...
		if ( $voucher_id > 0 ) {
			...
			if ( ! $voucher->has_status( 'voided' ) ) {
				wp_send_json_error( array( 'message' => __( 'Voucher is not voided', 'woocommerce-pdf-product-vouchers' ) ) );
			}
		...
		if ( current_user_can( 'manage_woocommerce' ) && check_admin_referer( 'vouchers-list-redeem-voucher' ) ) {
			...
			if ( $voucher && $voucher->is_editable() ) {
				...
				try {
					...
					wc_pdf_product_vouchers()->get_message_handler()->add_message( sprintf( __( 'Voucher %s redeemed.', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number() ) );

				} catch ( SV_WC_Plugin_Exception $e ) {

					/* translators: %1$s - voucher number, %2$s - error message */
					wc_pdf_product_vouchers()->get_message_handler()->add_error( sprintf( __( 'Could not redeem voucher %1$s: %2$s', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number(), $e->getMessage() ) );
				}
			}
		}
	...
	public function list_void_voucher() {
		...
		if ( current_user_can( 'manage_woocommerce' ) && check_admin_referer( 'vouchers-list-void-voucher' ) ) {
			...
			if ( $voucher && $voucher->is_editable() ) {
				...
				/* translators: %s - voucher number */
				wc_pdf_product_vouchers()->get_message_handler()->add_message( sprintf( __( 'Voucher %s voided.', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number() ) );
			}
		}

 ...
 * @since 3.4.0
 */
class WC_PDF_Product_Vouchers_Redemption_Handler {
	...
	public function __construct() {
		...
		// we can't effectively create virtual coupons before WC 3.2
		if ( SV_WC_Plugin_Compatibility::is_wc_version_gte( '3.2' ) ) {

			// TODO: consider adding support for subscriptions {IT 2017-03-29}

			// voucher coupon data handling
			add_filter( 'woocommerce_coupon_discount_types', array( $this, 'add_voucher_coupon_types' ) );
			add_filter( 'woocommerce_get_shop_coupon_data', array( $this, 'load_voucher_coupon_data' ), 10, 2 );
			add_filter( 'woocommerce_order_recalculate_coupons_coupon_object', array( $this, 'load_voucher_coupon_data_on_recalculation' ), 10, 2 );
			add_filter( 'woocommerce_coupon_is_valid', array( $this, 'is_voucher_coupon_valid' ), 10, 3 );
			...
			add_filter( 'woocommerce_calculated_total', array( $this, 'apply_multi_purpose_vouchers_to_cart' ) );
	...
	public function add_voucher_coupon_types( $coupon_types ) {
		...
		$coupon_types['multi_purpose_voucher'] = __( 'Multi-purpose voucher', 'woocommerce-pdf-product-vouchers' );
	 ...
	 * @since 3.4.0
	 *
	 * @param mixed $coupon_data an array of coupon data to load
	 * @param string $coupon_code the coupon code
	 * @return array an array of coupon data to load
	 */
	public function load_voucher_coupon_data( $coupon_data, $coupon_code ) {

		if ( empty( $coupon_data ) && $voucher = wc_pdf_product_vouchers_get_voucher( $coupon_code ) ) {
			...
			$coupon_data = array(
				...
				'limit_usage_to_x_items' => 'single' === $voucher->get_voucher_type() ? $voucher->get_product_quantity() : null,
	...
	public function load_voucher_coupon_data_on_recalculation( $coupon, $coupon_code ) {
		...
		if ( wc_pdf_product_vouchers_get_voucher( $coupon_code ) ) {
	...
	public function is_voucher_coupon_valid( $valid, $coupon, $discounts ) {
		...
		if ( $valid && $this->is_voucher_coupon( $coupon ) && $voucher = wc_pdf_product_vouchers_get_voucher( $coupon->get_code() ) ) {
			...
			if ( 'multi' === $voucher->get_voucher_type() ) {
				...
				$currency = $item instanceof WC_Order_Item ? $item->get_order()->get_currency() : get_woocommerce_currency();

				if ( $voucher->get_voucher_currency() !== $currency ) {
					throw new Exception( sprintf( __( 'Sorry, this voucher is only valid for the %s currency.', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_currency() ), 100 );
				}
			}
		...
		if ( $this->is_voucher_coupon( $coupon ) ) {
			switch ( $msg_code ) {
				case WC_Coupon::WC_COUPON_SUCCESS :
					$msg = __( 'Voucher applied successfully.', 'woocommerce-pdf-product-vouchers' );
				break;

				case WC_Coupon::WC_COUPON_REMOVED :
					$msg = __( 'Voucher removed successfully.', 'woocommerce-pdf-product-vouchers' );
				break;
			}
		}
	...
	public function voucher_coupon_label( $label, $coupon ) {
		...
		if ( $this->is_voucher_coupon( $coupon ) ) {
			$label = sprintf( __( 'Voucher: %s', 'woocommerce-pdf-product-vouchers' ), strtoupper( $coupon->get_code() ) );
		}
	...
	private function redeem_order_voucher( $order, $coupon_item ) {
		...
		$voucher = wc_pdf_product_vouchers_get_voucher( $coupon->get_code() );
		...
		if ( $coupon->is_type( 'percent' ) ) {
			...
			if ( ! empty( $discounts ) ) {
				foreach ( $discounts as $order_item_id => $discount ) {
					...
					$redeemed_qty += $discount / $item_price;
		...
		try {
			...
			$order->add_order_note( sprintf( __( 'Could not redeem voucher %1$s: %2$s', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number(), $e->getMessage() ) );
		}
	}
	...
	private function remove_order_voucher_redemption( $coupon_item, $order ) {
		...
		if ( $changed ) {
			try {
				...
				/* translators: %1$s - voucher number, %2$s - error message */
				$order->add_order_note( sprintf( __( 'Could not delete redemptions for voucher %1$s: %2$s', 'woocommerce-pdf-product-vouchers' ), $voucher->get_voucher_number(), $e->getMessage() ) );
			}
		}
	}
	 ...
	 * @return float $total
	 */
	public function apply_multi_purpose_vouchers_to_cart( $total = 0 ) {
	...
	public function add_order_totals_mpv_details_row( $total_rows, $order = null ) {
		...
		if ( $total_credit_used > 0 ) {
			...
			$mpv_details = array(
				'mpv_details' => array(
					'label' => __( 'Voucher credit used:', 'woocommerce-pdf-product-vouchers' ),
			...
			if ( array_key_exists( 'tax', $total_rows ) ) {
				$total_rows = SV_WC_Helper::array_insert_after( $total_rows, 'tax', $mpv_details );
	...
	public function output_admin_order_totals_mpv_details( $order_id = 0 ) {
		...
		<tr>
			<td class="label"><?php esc_html_e( 'Voucher credit used:', 'woocommerce-pdf-product-vouchers' ); ?> <span class="tips" data-tip="<?php esc_attr_e( 'This is the total credit used from multi-purpose-vouchers.', 'woocommerce-pdf-product-vouchers' ); ?>">[?]</span>:</td>
			<td width="1%"></td>
	...
	private function is_voucher_coupon( $coupon ) {
		return $coupon->is_type( 'multi_purpose_voucher' ) || wc_pdf_product_vouchers_get_voucher( $coupon->get_code() );
	}

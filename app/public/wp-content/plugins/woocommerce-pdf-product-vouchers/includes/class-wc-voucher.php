...
class WC_Voucher extends WC_Voucher_Base {
	...
	public function get_used_quantity() {
		return $this->get_product_quantity() - $this->get_remaining_quantity();
	}
	 ...
	 * @since 3.0.0
	 *
	 * @param float|array $amount (optional) a positive amount to redeem - if omitted, quantity must be provided and amount will be determined from quantity
	 * @param array $args {
	...
	public function redeem( $amount = null, $args = array() ) {
		...
		$redemption = $this->parse_redemption( $args );

		if ( ! $this->get_remaining_value() ) {
			throw new SV_WC_Plugin_Exception( __( 'No remaining value', 'woocommerce-pdf-product-vouchers' ) );
		}

		if ( ! $redemption['amount'] && empty( $redemption['quantity'] ) ) {
			throw new SV_WC_Plugin_Exception( __( 'Either amount or quantity must be provided', 'woocommerce-pdf-product-vouchers' ) );
		}

		if ( $redemption['amount'] && $redemption['amount'] < 0 ) {
			throw new SV_WC_Plugin_Exception( __( 'Redemption amount must be a positive number', 'woocommerce-pdf-product-vouchers' ) );
		}

		if ( $redemption['amount'] && 'single' === $this->get_voucher_type() && fmod( $redemption['amount'], $this->get_product_price() ) ) {
			throw new SV_WC_Plugin_Exception( __( 'Redemption amount must be a multiple of the product price for a single-purpose voucher.', 'woocommerce-pdf-product-vouchers' ) );
		}

		if ( $redemption['amount'] && $this->get_remaining_value() < $redemption['amount'] ) {
			throw new SV_WC_Plugin_Exception( __( 'Redemption amount is greater than remaining voucher value', 'woocommerce-pdf-product-vouchers' ) );
		}

		if ( $this->get_remaining_quantity() < $redemption['quantity'] ) {
			throw new SV_WC_Plugin_Exception( __( 'Redemption quantity is greater than remaining quantity', 'woocommerce-pdf-product-vouchers' ) );
		}
	...
	public function set_redemptions( $redemptions = array() ) {
		...
		foreach ( $redemptions as $i => $redemption ) {
			...
			if ( $is_single && fmod( $redemption['amount'], $this->get_product_price() ) ) {
				throw new SV_WC_Plugin_Exception( sprintf( '%s %s', $invalid_amount_message, __( 'Redemption amount must be a multiple of the product price for a single-purpose voucher.', 'woocommerce-pdf-product-vouchers' ) ) );
			}

			$redeemed += $redemption['amount'];

			if ( $redeemed > $value ) {
				throw new SV_WC_Plugin_Exception( __( 'Total amount of redemptions cannot be greater than voucher value.', 'woocommerce-pdf-product-vouchers' ) );
			}
	...
	private function parse_redemption( $args = array() ) {
		...
		if ( null !== $args['quantity'] ) {
			...
			if ( ! $args['amount'] ) {
				$args['amount'] = $this->get_product_price() * $args['quantity'];
			}
		}

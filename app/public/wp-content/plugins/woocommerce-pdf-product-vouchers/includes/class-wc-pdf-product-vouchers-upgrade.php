...
class WC_PDF_Product_Vouchers_Upgrade {
	...
	public static function run_update_scripts( $installed_version ) {
		...
		if ( ! empty( $installed_version ) ) {
			...
			$update_path = array(
				...
				'3.1.0' => 'update_to_3_1_0',
				'3.1.2' => 'update_to_3_1_2',
			);
	 ...
	 * @since 3.1.0
	 */
	private static function update_to_3_1_0() {
		...
		// 1. set voucher types
		self::set_voucher_types();

		// 2. set voucher customers
		self::set_voucher_customers();

		// 3. fix invalid voucher dates
		self::fix_legacy_voucher_dates();
	 ...
	 * @since 3.1.2
	 */
	private static function update_to_3_1_2() {
		global $wpdb;
			...
			foreach ( $order_ids as $order_id ) {
				...
				foreach ( $order->get_items() as $order_item_id => $item ) {
					...
					if ( ! empty( $item['voucher_id'] ) && ! isset( $item['wc_pdf_product_vouchers_is_converted'] ) ) {
						...
						// get order date
						$order_date = SV_WC_Order_Compatibility::get_date_created( $order, 'order_date' );
						...
						if ( ! empty( $old_redemptions ) ) {
							...
							try {
								...
								wc_pdf_product_vouchers()->log( 'Unable to redeem voucher: ' . $voucher->get_id() . ' - ' . $e->getMessage() );
							}
						}
	 ...
	 * @since 3.1.0
	 */
	private static function set_voucher_types() {
		global $wpdb;
		...
		$voucher_template_posts = get_posts( array(
		...
		do {
			...
			foreach ( $results as $result ) {
				...
				if ( 'single' === $type ) {
					...
					if ( fmod( $remaining_value, $product_price ) ) {
						$type = 'multi';
					}
				}
	 ...
	 * @since 3.1.0
	 */
	private static function set_voucher_customers() {
		global $wpdb;
	 ...
	 * @since 3.1.0
	 */
	private static function fix_legacy_voucher_dates() {
		global $wpdb;
	...
	private static function create_voucher_image_attachment( $attrs ) {
		...
		if ( ! $attachment_id || is_wp_error( $attachment_id ) ) {
			...
			$message = sprintf( __( 'Could not install default voucher image %1$s: %2$s', 'woocommerce-pdf-product-vouchers' ), $filename, $attachment_id->get_error_message() );

...
class WC_PDF_Product_Vouchers_PDF_Generator {
	...
	public static function generate_voucher_pdf( WC_Voucher $voucher, $save = true ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( "Cannot load voucher HTML: %s", 'woocommerce-pdf-product-vouchers' ), $response->get_error_message() ) );
		}
		...
		if ( isset( $response['response']['code'] ) && 200 !== (int) $response['response']['code'] ) {
			...
			throw new SV_WC_Plugin_Exception( sprintf( __( 'Cannot load voucher HTML: %1$d - %2$s', 'woocommerce-pdf-product-vouchers' ), $response['response']['code'], $response['response']['message'] ) );
		}

		$html = wp_remote_retrieve_body( $response );

		if ( empty( $html ) ) {
			throw new SV_WC_Plugin_Exception( __( 'Voucher HTML is empty', 'woocommerce-pdf-product-vouchers' ) );
		}

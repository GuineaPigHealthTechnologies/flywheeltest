...
class SA_WC_Chained_Products {
    ...
    public function admin_notice_sa_needs_wc_25_above() {
        ...
        <div class="updated error">
            <p><?php
                echo sprintf(__( '%s WooCommerce Chained Products is active but it will only work with WooCommerce 2.5+. %s.', self::$text_domain ), '<strong>' . __( 'Important', self::$text_domain ) . ':</strong>', '<a href="'.admin_url('plugins.php?plugin_status=upgrade').'" target="_blank" >' . __( 'Please update WooCommerce to the latest version', self::$text_domain ) . '</a>' );
            ?></p>
        </div>
        <?php
    }
    ...
    public function exclude_parent_variable_product_from_search() {
     	$search_result = ( isset( $_POST['result'] ) ) ? $_POST['result'] : '';
    ...
    public function add_order_item_meta_manually( $item_id, $item )  {
        if ( $this->is_wc_gte_30() ) {
	        ...
	        $item_to_add = $item->get_variation_id() ? ( $variation_id = $item->get_variation_id() ) : $item->get_product_id();
	    	$_product    = wc_get_product( $item_to_add );

		    if ( isset($_POST['chained_product_details'] ) )
			    $chained_product_details = $_POST['chained_product_details'];
	        ...
	        if ( ! empty ( $chained_product_details ) ) {
	            foreach ( $chained_product_details as $parent_id => $chained_values ) {
	                ...
	                if ( in_array( $item_to_add, $chained_product_ids) ) {
			        	...
			        	foreach( $args as $meta_key=>$meta_value ) {
			                if ( $meta_key == '_variation_data' && is_array( $meta_value ) ) {
	...
	public function on_add_order_item_manually() {
		...
		$item_to_add = $this->is_wc_gte_30() ?  wp_parse_id_list( is_array( $_POST['item_to_add'] ) ? $_POST['item_to_add'] : array( $_POST['item_to_add'] ) ) : sanitize_text_field( $_POST['item_to_add'] );
		$order_id    = absint( $_POST['order_id'] );
		$order       = wc_get_order( $order_id );

        if ( is_array( $item_to_add ) && $this->is_wc_gte_30() ) {

		    $chained_product_detail = $total_item_ids = $total_chained_product_details = $chained_product_ids = array();
        ...
        if ( $subscription instanceof WC_Subscription && ! empty( $parent_id ) ) {
	        ...
	        if ( ! empty( $chained_product_ids ) ) {
	            foreach ( $chained_product_ids as $chained_product_id  ) {
	                ...
	                $args = array(
		                        'totals'             => array( 'subtotal' => wc_format_decimal( 0 ), 'total' => wc_format_decimal( 0 ), 'subtotal_tax' => '', 'tax' => '' ),
	 ...
	 * @param WC_Order $order
	 * @param WC_Subscription $subscription
	 * @param int $add_line_item
	 * @param int $remove_line_item
     */
	public function update_chained_products_for_switched_subscription( $order, $subscription, $add_line_item, $remove_line_item ) {
        ...
        foreach( $subscription_items as $item_id => $item ) {
        	if ( isset( $item['chained_product_of'] ) ) {
	        	...
	        	if ( $product_id == $item['chained_product_of'] ){
    ...
    public function set_chained_product_property_in_order_item( $item, $cart_item_key, $values, $order  ) {
		if ( isset( $values['chained_item_of'] ) && ! empty( $values['chained_item_of'] ) ) {
	...
	public function enqueue_chained_products_js_css() {
		...
		if ( wp_script_is( 'select2' ) ) {
			wp_localize_script( 'select2', 'cp_select_params', array(
				'i18n_matches_1'            => _x( 'One result is available, press enter to select it.', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_matches_n'            => _x( '%qty% results are available, use up and down arrow keys to navigate.', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_no_matches'           => _x( 'No matches found', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_ajax_error'           => _x( 'Loading failed', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_input_too_short_1'    => _x( 'Please enter 1 or more characters', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_input_too_short_n'    => _x( 'Please enter %qty% or more characters', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_input_too_long_1'     => _x( 'Please delete 1 character', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_input_too_long_n'     => _x( 'Please delete %qty% characters', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_selection_too_long_1' => _x( 'You can only select 1 item', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_selection_too_long_n' => _x( 'You can only select %qty% items', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_load_more'            => _x( 'Loading more results&hellip;', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				'i18n_searching'            => _x( 'Searching&hellip;', 'enhanced select', SA_WC_Chained_Products::$text_domain ),
				...
				if ( ! empty( $product_detail ) ) {
					...
					?>
						<p class="form-field <?php if( ! isset( $product_detail[$product_id] ) ) echo 'nested_chained_products_'.$chained_parent_id; ?>" id="chained_products_row_<?php echo $chained_parent_id . '_' . $row_loop; ?>">
							...
							if ( $this->is_wc_gte_30() ) { ?>
                                <select class="wc-product-search" style="width: 50%;" id="chained_products_ids_<?php echo $chained_parent_id . '_' . $row_loop; ?>" name="<?php if( ! isset( $product_detail[$product_id] ) ) echo 'nested_'; ?>chained_products_ids[<?php echo $chained_parent_id; ?>][<?php echo $row_loop; ?>]" data-placeholder="<?php _e( 'Search for a product...', SA_WC_Chained_Products::$text_domain ); ?>"
									data-action="woocommerce_json_search_products_and_variations" data-exclude="<?php echo $chained_parent_id; ?>" <?php if( ! isset( $product_detail[$product_id] ) ) echo 'disabled';?> >
								    <?php
							...
							<input type="number" class="chained_products_quantity short" name="chained_products_quantity[<?php echo $chained_parent_id; ?>][<?php echo $row_loop; ?>]" value="<?php echo ( ! empty( $product_data['unit'] ) ) ? $product_data['unit'] : '1'; ?>" placeholder="<?php _e( 'Qty', self::$text_domain ); ?>" min="1" <?php  if ( ! isset( $product_detail[$product_id] ) ) echo 'readonly'; ?>/>
							<?php
							if( isset( $product_detail[$product_id] ) ) {
					...
					<p class="form-field" id="chained_products_row_<?php echo $chained_parent_id . '_' . $row_loop; ?>">
						...
						if ( $this->is_wc_gte_30() ) { ?>
						    <select class="wc-product-search" style="width: 50%;" id="chained_products_ids_<?php echo $chained_parent_id . '_' . $row_loop; ?>" name="chained_products_ids[<?php echo $chained_parent_id; ?>][<?php echo $row_loop; ?>]" data-placeholder="<?php _e( 'Search for a product...', SA_WC_Chained_Products::$text_domain ); ?>"
                                data-action="woocommerce_json_search_products_and_variations" data-exclude="<?php echo $chained_parent_id; ?>" /></select> <?php
                        }
                        else { ?>
	                        <input type="hidden" class="wc-product-search" style="width: 50%;" id="chained_products_ids_<?php echo $chained_parent_id . '_' . $row_loop; ?>" name="chained_products_ids[<?php echo $chained_parent_id; ?>][<?php echo $row_loop; ?>]" data-placeholder="<?php _e( 'Search for a product...', SA_WC_Chained_Products::$text_domain ); ?>"
				...
				jQuery('#add_chained_products_row_'+chained_id).off('click').on( 'click', function() {
					...
					var new_row = " <p class='form-field' id='chained_products_row_"+chained_id+"_"+row_id+"'>\
										...
										<?php if ( $this->is_wc_gte_30() ){?>\
											<select class='wc-product-search' style='width: 50%;' id='chained_products_ids_"+chained_id+"_"+row_id+"'  name='chained_products_ids["+chained_id+"]["+row_id+"]' data-placeholder='<?php _e( 'Search for a product...', SA_WC_Chained_Products::$text_domain ); ?>' \
											data-action='woocommerce_json_search_products_and_variations' data-exclude='"+chained_id+"'/></select>\
										<?php } else { ?>\
											<input type='hidden' class='wc-product-search' style='width: 50%;' id='chained_products_ids_"+chained_id+"_"+row_id+"' name='chained_products_ids["+chained_id+"]["+row_id+"]' data-placeholder='<?php _e( 'Search for a product...', SA_WC_Chained_Products::$text_domain ); ?>' \
											data-action='woocommerce_json_search_products_and_variations' data-exclude='<?php echo $total_ids_to_exclude; ?>' data-multiple='true'/>\
										<?php } ?>\
	...
	public function on_process_product_meta( $post_id, $post ) {
		...
		// save variations
		if ( in_array( $_POST['product-type'], $this->get_supported_product_types() ) ) {		// save supported product types
	...
	public function update_chained_product_data( $chained_parent_id ) {
		if( isset( $_POST['chained_products_ids'][$chained_parent_id] ) && ! empty( $_POST['chained_products_ids'][$chained_parent_id] ) ) {
			...
			$chained_products_ids 			= array_filter( $_POST['chained_products_ids'][$chained_parent_id] );

			if ( empty( $chained_products_ids ) ) return;

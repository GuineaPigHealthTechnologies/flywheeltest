...
class WC_Subscription_Downloads_Products {
	...
	public function save_simple_product_data( $product_id ) {
		if ( ! isset( $_POST['_downloadable'] ) || 'publish' !== get_post_status( $product_id ) ) {
			update_post_meta( $product_id, '_subscription_downloads_ids', $_POST['_subscription_downloads_ids'] );
			return;
		}

		delete_post_meta( $product_id, '_subscription_downloads_ids', $_POST['_subscription_downloads_ids'] );

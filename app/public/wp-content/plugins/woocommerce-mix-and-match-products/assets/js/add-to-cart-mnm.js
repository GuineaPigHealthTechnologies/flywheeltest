...
jQuery( document ).ready( function($) {
	...
	$.fn.wc_mnm_form = function() {
		...
		wc_mnm_scripts[ container_id ] = {
			...
			$mnm_reset: 		  $form.find( '.mnm_reset' ),

			min_container_size:   $form_cart.data( 'min_container_size' ),
			max_container_size:   $form_cart.data( 'max_container_size' ),
			...
			bind_event_handlers: function() {
				...
				container.$mnm_items
					...
					.on( 'input change', '.qty', function( e ) {
						container.update( e.type );
					} );
			...
			/**
			 * Get Min MnM container size.
			 */

			get_min_container_size: function() {

				if ( typeof( this.min_container_size ) !== 'undefined' && this.min_container_size !== '' ) {
					return parseInt( this.min_container_size );
				}
			...
			/**
			 * Get Max MnM container size.
			 */

			get_max_container_size: function() {

				if ( typeof( this.max_container_size ) !== 'undefined' && this.max_container_size !== '') {
					return parseInt( this.max_container_size );
				}
			...
			update: function( event_type ) {

				var container           = this;
    			var min_container_size  = this.get_min_container_size();
    			var max_container_size  = this.get_max_container_size();
				...
				this.$mnm_items.each( function() {
					...
					if ( $input.length > 0 ) {
						...
						if ( max_container_size > 0 ) {
							max = Math.min( max, parseFloat( max_container_size ) );
						}

						if ( 'change' === event_type && min >= 0 && ( parseFloat( $input.val() ) < min || isNaN( parseFloat( $input.val() ) ) ) ) {
							$input.val( min );
						}

						if ( 'change' === event_type && max > 0 && parseFloat( $input.val() ) > max ) {
							$input.val( max );
						}
